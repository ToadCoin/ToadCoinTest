...
{
    // Wait for IRC to get it first - disabled with toadcoin
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Toadcoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are toadcoin dns seeds.
static const char *strDNSSeed[][2] = {
    // NOTE: Only testnet nodes are included in this build. If you are using this
    // for a real Toadcoin offering, you will want to remove the "t" that
    // appears before the node name and IP
    {"tToadcoin NODE_2_EU-Ireland", "t54.72.105.68"},
    {"tToadcoin NODE_3_SE-A_Singapore", "t54.255.142.16"},
};
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  Toadcoin is probably already running."), ntohs(sockaddr.sin_port));
        else
