...
//
// Global state//
static const uint256 hashGenesisBlockOfficial("0x00000a2b2b3abbe00576c0f7575f06e5af3a941a5c5c38ca849196a8253092f1");
static const uint256 hashGenesisBlockTestNet ("0x000001e21a746aee33e7c378b019bf9b39eb5102f44bfad7f7cf09749cf395ac");
...
int nCoinbaseMaturity = COINBASE_MATURITY_TOAD;
...
const string strMessageMagic = "Toadcoin Signed Message:\n";
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
    if (!fConnect)
    {
        // Toadcoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // Toadcoin: enforce minimum output amount
...
// toadcoin: minter's coin stake is rewarded based on coin age spent (coin-days)
...
bool CBlock::AcceptBlock()
{
    ...
    // Toadcoin: check switch from proof of work to proof of stake
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (pblock->IsProofOfStake())
    {
        ...
        if (!CheckProofOfStake(pblock->vtx[1], pblock->nBits, hashProofOfStake))
        {
            ...
            // toadcoin: ask for missing blocks
            if (pfrom)
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "Toadcoin", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Toadcoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "The Greatest Toad To Ever Live!";
        unsigned int nTimeGenesis=1501710990;
        unsigned int nNonceGenesis=1501710;

        if (fTestNet)
        {
            pszTimestamp="I Swear We Are Not Frogs!!";
            nTimeGenesis=1501710990;
            nNonceGenesis=1501710;
        }
        ...
        printf("Toadcoin Genesis Block Found:\n");
        printf("genesis hash=%s\n", block.GetHash().ToString().c_str());
        printf("merkle root=%s\n", block.hashMerkleRoot.ToString().c_str());
       
        block.print();
     
        printf("End Toadcoin Genesis Block\n");
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        if (!fTestNet)
            assert(block.hashMerkleRoot == uint256("0xf59622645234ec9df657cb1aa1c683cd08cc3e3be4644eff32ec5ac586143df7"));
        else
            assert(block.hashMerkleRoot == uint256("0x235ce4a18de455fc8b2f5d146fff5eaa3b25303a782f5620608e9a7cb8e2caa8"));
...
string GetWarnings(string strFor)
{
    ...
    {
        nPriority = 100;
        strStatusBar = "WARNING: Checkpoint is too old. Please wait for the block chain to download, or notify developers of the issue (https://github.com/Toadcoin/Toadcoin/issues).";
    }
    ...
    {
        nPriority = 3000;
        strStatusBar = strRPC = "WARNING: Invalid checkpoint found! Displayed transactions may not be correct! You may need to upgrade, or notify developers of the issue (https://github.com/Toadcoin/Toadcoin/issues).";
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave)
                pfrom->AskFor(inv, IsInitialBlockDownload()); // toadcoin: immediate retry during initial download
